 ...
 * fetch data for, in override > farm > default precedence. The returned
 * state/district always echo back what was picked/typed even when
 * unsupported (isSupported: false) — callers show an honest empty state
 * rather than the resolver silently substituting a different location.
 */
export function resolveEffectiveLocation(params: {
    ...
    const { farm, override, locations } = params;
